...
namespace _929_Bilt2020_PlaypenParent
{
    ...
    public class Invoke1819_UnderstandingTransforms : IExternalCommand
    {
                ...
                foreach (Type objType in myIEnumerableType)
                {
                    if (objType.IsClass)
                    {
                        if (objType.Name.ToLower() == strCommandName.ToLower())
                        {
                            ...
                            result = objType.InvokeMember("UnderstandingTransforms", BindingFlags.Default | BindingFlags.InvokeMethod, null, ibaseObject, arguments);
    ...
    [Autodesk.Revit.Attributes.Transaction(Autodesk.Revit.Attributes.TransactionMode.Manual)]
    public class DevInvoke1819_UnderstandingTransforms : IExternalCommand
    {
        public string dllModuleName { get; set; } = "_929_Bilt2020_PlaypenChild";
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
